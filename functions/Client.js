// TODO fix dependency
import { level } from "../src/helpers/Level.js";
...
export class Client {
  ...
  async getPermissions() {
    try {
      const publicLevel = await this.db
        .getDoc(this.db.doc(this.db, `timetables/${this.table}`))
        .then(
          (table) =>
  ...
  async setPublicLevel(level) {
    try {
      const timetableDoc = this.db.doc(this.db, `timetables/${this.table}`);
      const snapshot = await this.db.getDoc(timetableDoc);

      if (snapshot.exists())
        await this.db.updateDoc(timetableDoc, {
          publicLevel: level,
        });
      else
        await this.db.setDoc(timetableDoc, {
          publicLevel: level,
        });
  ...
  async getTimetable() {
    try {
      const snapshot = await this.db.getDoc(
        this.db.doc(`timetables/${this.table}`),
      );
